...
WALMART_VISUAL_FIX_VERSION = "2026.07.16.4"
        ...
        script = r"""
            ...
            for (const node of document.querySelectorAll("div,span,p,h3,h4")) {
                ...
                for (
                    ...
                    // Prefer the smallest card that includes a meaningful
                    // inventory status. Walmart often puts "Low stock" one
                    // wrapper above the arrival text, so stopping at the
                    // first "Arrives" or "Today" node loses that status.
                    let score = 0;
                    ...
                    if (
                        /out of stock|not available|unavailable|sold out/i
                            .test(value)
                    ) {
                        score += 80;
                    }
                    ...
                    if (
                        /arrives|delivery date|order within|free shipping|today|tomorrow|get it nearby|ready in|check nearby/i
                            .test(value)
                    ) {
                        score += 40;
                    }

                    // Smaller containers are more likely to be the actual
                    // fulfillment card rather than the whole buy box.
                    score -= Math.min(value.length, 600) / 1000;
