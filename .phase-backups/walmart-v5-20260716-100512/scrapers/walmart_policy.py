...
WALMART_POLICY_VERSION = "2026.07.16.4"
...
class WalmartPolicy:
    ...
    def classify_rendered_signals(
        ...
        if enabled_cta:
            # Walmart frequently renders unavailable alternative colors,
            # conditions (for example Open Box), or seller offers inside the
            # same purchase region. Their standalone "Out of stock" text must
            # not defeat a valid selected-item Add to cart button.
            #
            # The exception is when every detected fulfillment method is
            # unavailable. In that case the apparent CTA is stale or belongs
            # to a different offer/card and the selected item is OOS.
            ...
            if all_fulfillment_unavailable:
                return (
                    ...
                    None,
                    (
                        "An Add to cart control was present, but every "
                        "detected primary fulfillment method is unavailable; "
                        "the control is treated as stale or unrelated."
                    ),
                    False,
                    "all_fulfillment",
                )
